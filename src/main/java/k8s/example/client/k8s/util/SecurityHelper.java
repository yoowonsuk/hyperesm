...
public class SecurityHelper {
    ...
    private static X509Certificate createX509CertificateFromString(
        	final String certificateString) throws IOException, CertificateException 
        {
         	final CertificateFactory certificateFactoryX509 = CertificateFactory.getInstance("X.509");
         	
         	final InputStream inputStream = new ByteArrayInputStream(certificateString.getBytes());
            final X509Certificate certificate = (X509Certificate) certificateFactoryX509.generateCertificate(inputStream);
            inputStream.close();
    ...
    private static PrivateKey createPrivateKeyFromPemString(final String keyFileString) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException 
    {
    	// Loads a privte key from the specified key file name
        final PemReader pemReader = new PemReader(new StringReader(keyFileString));
        final PemObject pemObject = pemReader.readPemObject();
        final byte[] pemContent = pemObject.getContent();
        pemReader.close();
        final PKCS8EncodedKeySpec encodedKeySpec = new PKCS8EncodedKeySpec(pemContent);
	...
	private static KeyManagerFactory createKeyManagerFactory(
		final String clientCertificateString, final String clientKeyString, final String clientKeyPassword) 
		throws InvalidKeySpecException, NoSuchAlgorithmException, KeyStoreException, IOException, CertificateException, UnrecoverableKeyException
	{
		...
		final X509Certificate clientCertificate = createX509CertificateFromString(clientCertificateString);	
		// Load the private client key
		final PrivateKey privateKey = createPrivateKeyFromPemString(clientKeyString);
	...
	private static TrustManagerFactory createTrustManagerFactory(
		final String caCertificateString) 
		throws CertificateException, NoSuchAlgorithmException, IOException, KeyStoreException 
	{
		// Creates a trust manager factory
		// Load CA certificate
		final X509Certificate caCertificate = (X509Certificate) createX509CertificateFromString(caCertificateString);
